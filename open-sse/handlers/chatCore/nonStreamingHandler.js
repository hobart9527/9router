 ...
 * Target format is provider format; Source format is client format.
 */
export function translateNonStreamingResponse(responseBody, targetFormat, sourceFormat, customToolNames = null) {
  if (targetFormat === sourceFormat) return responseBody;

  let normalizedResponse = responseBody;

  // 1. Normalize provider response to OpenAI chat.completion format if it is not already
  if (targetFormat === FORMATS.OPENAI) {
    normalizedResponse = responseBody;
  } else if (targetFormat === FORMATS.OPENAI_RESPONSES) {
    // Provider returned a Responses body (e.g. codex non-stream): fold it back to
    // chat.completion so the sourceFormat branch below can reach Claude/Gemini.
    normalizedResponse = responsesJsonToChatCompletion(responseBody);
  } else if (targetFormat === FORMATS.GEMINI || targetFormat === FORMATS.ANTIGRAVITY || targetFormat === FORMATS.GEMINI_CLI || targetFormat === FORMATS.VERTEX) {
    ...
    normalizedResponse = ollamaBodyToOpenAI(responseBody);
  }

  // 2. Convert normalized OpenAI response to client's requested sourceFormat
  if (sourceFormat === FORMATS.OPENAI_RESPONSES) {
    return openAICompletionToResponses(normalizedResponse, customToolNames);
  }
  if (sourceFormat === FORMATS.CLAUDE) {
    return openAICompletionToClaudeMessage(normalizedResponse);
  }
